...
- Канальный (data link) - Физическая адресация (Wi-Fi, Ethernet, DSL) **PDU - кадры (последовательность бит длиной n)**
...
По сути почти OSI:
- Прикладной - **PDU - данные (сообщение)**
- Транспортный - **PDU - сегменты**
- Сетевой - **PDU - датаграммы**
- Канальный - **PDU - кадры**
- Физический - **PDU - биты (ед. обмена)**

Передача данных:
> Хост передаёт сообщение &rightarrow; Прикладной &rightarrow; Транспортный &rightarrow; Сетевой &rightarrow; Канальный &rightarrow; Физический &rightarrow; Канальный &rightarrow; Сетевой &rightarrow; Транспортный &rightarrow; Прикладной &rightarrow; Хост получает сообщение

# TCP - Transmission Control Protocol
Механизм TCP предоставляет поток данных с предварительной установкой соединения, осуществляет повторный запрос данных в случае потери данных и устраняет дублирование при получении двух копий одного пакета &Rightarrow; целостность данных.

## Заголовок сегмента TCP
![tcp](./images/tcp_header.png)
...
Используется в стримминге, онлайн играх и т.п.

## Заголовок датаграммы UDP
![tcp](./images/udp_header.png)
...
- Канальный (data link) - Физическая адресация (Wi-Fi, Ethernet, DSL)
