...
- $\displaystyle T = T_{conn} + \frac{S}{R}$, где $T_{conn} = const$ - время установления соединения, $S$ - объем данных, $R$ - пропускная способность
- Передача данных без установления логического соединения, $\displaystyle T = \frac{S+h}{R}$, где $h = const$ - заголовок пакета
